...
const LIKELIHOOD_CONFIG = {
  more_likely: { label: "More Likely", color: "#06B6D4", barColor: "bg-cyan-500", textColor: "text-cyan-600 dark:text-cyan-400" },
  possible:    { label: "Possible",    color: "#10B981", barColor: "bg-emerald-500", textColor: "text-emerald-600 dark:text-emerald-400" },
  less_likely: { label: "Less Likely", color: "#F59E0B", barColor: "bg-amber-500", textColor: "text-amber-600 dark:text-amber-400" },
  rule_out:    { label: "Rule Out",    color: "#EF4444", barColor: "bg-red-500", textColor: "text-red-600 dark:text-red-400" },
};
...
function CopyButton({ text, label = "Copy" }) {
  ...
  return (
    <button onClick={handleCopy} className="flex items-center gap-1.5 rounded-full border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 px-3 py-1.5 text-[11px] font-bold text-slate-600 dark:text-white/60 hover:border-cyan-500 dark:hover:border-cyan-400/40 hover:text-cyan-700 dark:hover:text-cyan-300 transition-all">
...
function ImpactBar({ pct, color }) {
  return (
    <div className="flex items-center gap-2 min-w-[120px]">
      ...
      <span className="text-[11px] font-mono font-bold text-slate-500 dark:text-white/50 w-8 text-right tabular-nums">{pct}%</span>
    </div>
  );
}
  ...
  return (
    <button
      ...
      className={`relative whitespace-nowrap rounded-full px-4 py-2 text-xs font-bold transition-all ${
        active
          ? "bg-cyan-500 dark:bg-cyan-500 text-white shadow-lg shadow-cyan-500/25"
          : "bg-slate-100 dark:bg-white/5 text-slate-600 dark:text-white/50 hover:bg-slate-200 dark:hover:bg-white/10 hover:text-slate-900 dark:hover:text-white/80"
      } ${className}`}
    >
      {children}
    </button>
  ...
  return (
    <div className="space-y-5 text-slate-900 dark:text-white">
      ...
      <div className="rounded-3xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#0E141D] p-6 sm:p-8 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setReceiptOpen(true)} className="flex items-center gap-1.5 rounded-full border border-cyan-300 dark:border-cyan-400/30 bg-cyan-50 dark:bg-cyan-400/10 px-3.5 py-1.5 text-xs font-bold text-cyan-800 dark:text-cyan-300 hover:bg-cyan-100 dark:hover:bg-cyan-400/20 transition-colors font-mono">
              <Receipt className="h-3.5 w-3.5" /> <span>Receipt</span>
            </button>
            <button onClick={() => setShareOpen(true)} className="flex items-center gap-1.5 rounded-full border border-slate-200 dark:border-white/10 bg-slate-100 dark:bg-white/[0.03] px-3.5 py-1.5 text-xs font-medium text-slate-700 dark:text-white/70 hover:border-cyan-500/40 hover:text-slate-900 dark:hover:text-white transition-colors">
        ...
        {data.what_this_means_for_you && (
          <p className="mt-3 text-sm text-slate-700 dark:text-white/70 leading-relaxed border-t border-slate-100 dark:border-white/5 pt-3">
            {data.what_this_means_for_you}
          </p>
        )}
      </div>
      ...
      <Panel className="p-5 sm:p-7">
        <div className="flex items-center gap-2 text-cyan-700 dark:text-cyan-400 text-xs font-bold uppercase tracking-wider font-mono mb-4">
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          {/* Primary Trigger */}
          <div className="rounded-2xl border border-cyan-200 dark:border-cyan-500/20 bg-cyan-50/60 dark:bg-cyan-500/[0.05] p-4">
            <div className="text-[10px] uppercase tracking-wider text-cyan-600 dark:text-cyan-400 font-bold font-mono mb-1.5">⚡ Primary Trigger</div>
            <p className="text-sm font-bold text-slate-900 dark:text-white leading-snug">
              {data.quick_take ? data.quick_take.split(/[.!]/)[0] + "." : data.title}
            </p>
          </div>
          {/* Core Biological Lever */}
          <div className="rounded-2xl border border-violet-200 dark:border-violet-500/20 bg-violet-50/60 dark:bg-violet-500/[0.05] p-4">
            <div className="text-[10px] uppercase tracking-wider text-violet-600 dark:text-violet-400 font-bold font-mono mb-1.5">🧬 Core Mechanism</div>
            <p className="text-sm font-bold text-slate-900 dark:text-white leading-snug">
              {data.title || "Physiological Pathway Under Investigation"}
            </p>
          </div>
          {/* First-Line Fix */}
          <div className="rounded-2xl border border-emerald-200 dark:border-emerald-500/20 bg-emerald-50/60 dark:bg-emerald-500/[0.05] p-4">
            <div className="text-[10px] uppercase tracking-wider text-emerald-600 dark:text-emerald-400 font-bold font-mono mb-1.5">☀️ First-Line Fix</div>
            <p className="text-sm font-bold text-slate-900 dark:text-white leading-snug">
        ...
        {/* Verdict Banner */}
        <div className="mt-4 rounded-2xl bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 dark:from-white/[0.06] dark:via-white/[0.03] dark:to-white/[0.06] p-4 border border-slate-700 dark:border-white/10">
          <p className="text-sm font-bold text-white dark:text-white/90 text-center leading-relaxed">
      ...
      {/* ━━━━━━ 3. FOR YOU (Personalized — keep existing compact grid) ━━━━━━ */}
      {data.for_you && typeof data.for_you === "object" && data.for_you.biggest_opportunity && (
        <div className="rounded-3xl border border-cyan-300 dark:border-cyan-400/30 bg-cyan-50/50 dark:bg-gradient-to-br dark:from-cyan-400/[0.05] dark:to-transparent p-5 sm:p-6 space-y-3 shadow-sm">
          <div className="flex items-center gap-2 text-cyan-700 dark:text-cyan-300 text-xs font-bold uppercase tracking-wider font-mono">
            <Heart className="h-4 w-4 text-cyan-600 dark:text-cyan-400" /> <span>For You</span>
          </div>
          <div className="grid gap-2.5 sm:grid-cols-2">
            {[
              { label: "Biggest Opportunity", value: data.for_you.biggest_opportunity, color: "text-cyan-700 dark:text-cyan-400" },
              { label: "Focus First", value: data.for_you.focus_first, color: "text-emerald-700 dark:text-emerald-400" },
              { label: "Probably Don't Worry About", value: data.for_you.dont_need_to_worry_about, color: "text-slate-500 dark:text-white/40" },
              { label: "Investigate", value: data.for_you.could_investigate, color: "text-amber-700 dark:text-amber-300" },
            ].filter(item => item.value).map((item, i) => (
              <div key={i} className="rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-black/40 p-3.5 text-xs shadow-sm">
                <span className={`${item.color} font-bold text-[10px] uppercase tracking-wider block font-mono`}>{item.label}</span>
                <p className="text-slate-800 dark:text-white/85 leading-relaxed font-normal mt-1">{item.value}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <Panel className="p-5 sm:p-7">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <span className="flex h-6 w-6 items-center justify-center rounded-full bg-cyan-500 dark:bg-cyan-400 text-white dark:text-black text-xs font-extrabold font-mono">✓</span>
            ...
            {timelineTab === "month" && (
              <>
                ...
                {data.action_plan?.when_doctor && (
                  <div className="rounded-2xl border border-amber-200 dark:border-amber-500/20 bg-amber-50 dark:bg-amber-500/[0.05] p-3.5 text-xs">
                    <span className="text-amber-700 dark:text-amber-400 font-bold font-mono text-[10px] uppercase tracking-wider">When to See a Doctor</span>
                    <p className="text-slate-800 dark:text-white/85 mt-1 leading-relaxed">{data.action_plan.when_doctor}</p>
                  </div>
                )}
              </>
            )}
        ...
        <Panel className="p-5 sm:p-7">
          ...
          <div className="space-y-1.5">
            {filteredCauses.map((rc, i) => {
              ...
              return (
                <div key={`${causeFilter}-${i}`} className="rounded-2xl border border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/[0.02] overflow-hidden transition-colors hover:border-slate-200 dark:hover:border-white/10">
                  <button
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1.5">
                        <span className="text-sm font-bold text-slate-900 dark:text-white/95 truncate">{rc.category}</span>
                    ...
                    <ChevronDown className={`h-4 w-4 text-slate-400 dark:text-white/30 shrink-0 transition-transform ${isOpen ? "rotate-180" : ""}`} />
                  </button>
                    ...
                    {isOpen && (
                      ...
                      >
                        <div className="px-3.5 pb-3.5 space-y-2">
                          <p className="text-xs text-slate-600 dark:text-white/70 leading-relaxed">{rc.findings}</p>
                          <div className="rounded-xl border border-slate-200 dark:border-white/5 bg-white dark:bg-black/40 p-2.5 text-[11px]">
                            <strong className="text-cyan-700 dark:text-cyan-300 font-bold">Action: </strong>
                            <span className="text-slate-700 dark:text-white/70">{rc.remedy}</span>
                          </div>
                        </div>
                      </motion.div>
                    )}
        ...
        <AnimatePresence mode="wait">
              ...
              {data.nutritional_gaps && data.nutritional_gaps.length > 0 && (
                <div className="mb-4">
                        ...
                        <button
                          ...
                          className={`rounded-full px-3.5 py-1.5 text-xs font-bold transition-all border ${
                            selectedGap === i
                              ...
                              : "bg-slate-100 dark:bg-white/5 text-slate-700 dark:text-white/60 border-slate-200 dark:border-white/10 hover:border-cyan-400"
                          }`}
                        >
                          <span className={`inline-block h-1.5 w-1.5 rounded-full mr-1.5 ${selectedGap === i ? "bg-white" : rel.dot}`} />
                          {ng.nutrient}
                        </button>
                      );
                    })}
                  ...
                  {(() => {
                    ...
                    if (!item) return null;
                    return (
                      <div className="rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/[0.02] p-5 space-y-3">
                        ...
                        <p className="text-sm text-slate-700 dark:text-white/80 leading-relaxed">{item.why_it_matters}</p>
                        ...
                        <div className="grid gap-2 sm:grid-cols-2 text-xs">
                          {[
                            ...
                            <div key={idx} className="rounded-xl border border-slate-200 dark:border-white/5 bg-white dark:bg-black/40 p-3 shadow-sm">
                              <span className="text-slate-500 dark:text-white/40 uppercase text-[10px] tracking-wider block font-mono font-bold">{d.label}</span>
                              <span className="text-slate-800 dark:text-white/85 mt-1 block font-medium">{d.value}</span>
                            </div>
                          ))}
                        </div>

                        {item.supplement_status && (
                          <div className="text-[11px] text-slate-500 dark:text-white/50 font-mono">
                            Supplement verdict: <span className="text-slate-800 dark:text-white/80 font-bold">{item.supplement_status}</span>
                          </div>
                        )}
                      </div>
                    );
              ...
              {data.food_solutions && data.food_solutions.length > 0 && (
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-xs font-bold uppercase tracking-wider text-emerald-700 dark:text-emerald-400 font-mono">Top Food Sources</span>
                  </div>
                  <div className="grid gap-2.5 sm:grid-cols-2 lg:grid-cols-3">
                    {data.food_solutions.map((fs, i) => (
                      <div key={i} className="rounded-2xl border border-emerald-200 dark:border-emerald-500/15 bg-emerald-50/40 dark:bg-emerald-500/[0.03] p-4 text-xs space-y-1.5">
                        <div className="font-bold text-sm text-emerald-900 dark:text-emerald-300">{fs.food || fs.nutrient}</div>
                        <div className="text-slate-700 dark:text-white/70 leading-relaxed">
                          {fs.key_nutrients || fs.best_foods}
                        </div>
                        <div className="grid grid-cols-2 gap-2 text-[11px] pt-1">
                          <div>
                            <span className="text-slate-500 dark:text-white/40 block">Serving</span>
                            <span className="text-slate-800 dark:text-white/75 font-medium">{fs.serving || fs.serving_size}</span>
                          </div>
                          <div>
                            <span className="text-slate-500 dark:text-white/40 block">Mechanism</span>
                            <span className="text-slate-800 dark:text-white/75 font-medium">{fs.mechanism || fs.nutrient_contribution}</span>
                          </div>
                        </div>
                        {(fs.timing || fs.absorption_tips) && (
                          <div className="rounded-xl border border-slate-200 dark:border-white/5 bg-white dark:bg-black/40 p-2 text-[11px]">
                            <span className="text-emerald-700 dark:text-emerald-400 font-bold">Tip: </span>
                            <span className="text-slate-700 dark:text-white/70">{fs.timing || fs.absorption_tips}</span>
                          </div>
                        )}
                      </div>
                    ))}
          ...
          {coreTab === "supplement" && (
            <motion.div key="supplement" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.2 }}>
              <div className="flex items-center gap-2 mb-3">
                <Pill className="h-4 w-4 text-cyan-600 dark:text-cyan-400" />
                <span className="text-xs font-bold uppercase tracking-wider text-cyan-700 dark:text-cyan-400 font-mono">Evidence-Based Supplement Audit</span>
              </div>
              <p className="text-xs text-slate-500 dark:text-white/50 mb-4">Supplements evaluated by evidence strength and safety — never pushed unnecessarily.</p>
              ...
              {data.supplement_priorities && data.supplement_priorities.length > 0 ? (
                <div className="space-y-2.5">
                  {data.supplement_priorities.map((sp, i) => {
                    ...
                    return (
                      <div key={i} className={`rounded-2xl border ${borderClass} ${bgClass} p-4 text-xs`}>
                        <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                          <div>
                            ...
                            <span className={`text-[10px] uppercase font-bold tracking-wider font-mono ${
                              isNotRec ? "text-red-600 dark:text-red-400" : isPri1 ? "text-emerald-600 dark:text-emerald-400" : "text-amber-600 dark:text-amber-400"
                            }`}>
                        ...
                        <p className="text-slate-700 dark:text-white/75 leading-relaxed mb-2">{sp.why_this || sp.rationale}</p>

                        {!isNotRec && (
                          <div className="grid grid-cols-3 gap-2 text-[11px]">
                            <div className="rounded-lg bg-white dark:bg-black/40 p-2 border border-slate-100 dark:border-white/5">
                              <span className="text-slate-400 dark:text-white/30 block text-[9px] uppercase font-mono">Form</span>
                              <span className="text-slate-900 dark:text-white/90 font-bold">{sp.form}</span>
                            </div>
                            <div className="rounded-lg bg-white dark:bg-black/40 p-2 border border-slate-100 dark:border-white/5">
                              <span className="text-slate-400 dark:text-white/30 block text-[9px] uppercase font-mono">Dose</span>
                              <span className="text-slate-900 dark:text-white/90 font-bold">{sp.dosage || sp.dose}</span>
                            </div>
                            <div className="rounded-lg bg-white dark:bg-black/40 p-2 border border-slate-100 dark:border-white/5">
                              <span className="text-slate-400 dark:text-white/30 block text-[9px] uppercase font-mono">Timing</span>
                ...
                <div className="text-center py-8 text-slate-400 dark:text-white/30 text-sm">No supplement data available for this query.</div>
              )}
            </motion.div>
          )}
          ...
          {coreTab === "biomarker" && (
            <motion.div key="biomarker" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.2 }}>
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs font-bold uppercase tracking-wider text-cyan-700 dark:text-cyan-400 font-mono">Tests & Biomarkers to Discuss</span>
              </div>
              ...
              {data.biomarkers && data.biomarkers.length > 0 ? (
                <div className="grid gap-2.5 sm:grid-cols-2">
                  {data.biomarkers.map((bm, i) => (
                    <div key={i} className="rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/[0.02] p-4 text-xs space-y-2 shadow-sm">
                      <div className="flex items-center justify-between">
                        <div className="text-sm font-bold text-cyan-800 dark:text-cyan-300 font-mono">{bm.test_name || bm.marker}</div>
                        <CopyButton
                      ...
                      <div className="grid grid-cols-2 gap-2 text-[11px]">
                        <div>
                          ...
                          <span className="text-slate-700 dark:text-white/75">{bm.measures || bm.what_it_measures}</span>
                        </div>
                        <div>
                          <span className="text-slate-500 dark:text-white/40 block text-[10px] uppercase font-mono font-bold">Why It Matters</span>
                          <span className="text-slate-700 dark:text-white/75">{bm.why_it_matters || bm.matters}</span>
                        </div>
                      </div>
                      {bm.limitations && (
                        <div className="rounded-xl border border-slate-200 dark:border-white/5 bg-white dark:bg-black/40 p-2.5 text-[11px] text-slate-600 dark:text-white/60">
                          <strong className="text-slate-800 dark:text-white/80">Limitations:</strong> {bm.limitations}
                        </div>
                      )}
                    </div>
                  ))}
                ...
                <div className="text-center py-8 text-slate-400 dark:text-white/30 text-sm">No biomarker data available for this query.</div>
              )}
            </motion.div>
          )}
                    ...
                    <div className="space-y-2 pt-2">
                      ...
                      {research.studies.map((st, i) => (
                        <div key={i} className="rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-white/[0.02] p-3 text-xs shadow-sm">
                          ...
                          <p className="mt-1.5 text-slate-600 dark:text-white/70">{st.main_result}</p>
                          {st.url && (
                            <a href={st.url} target="_blank" rel="noopener noreferrer" className="mt-2 inline-flex items-center gap-1 text-[11px] text-cyan-700 dark:text-cyan-400 hover:underline font-bold">
      ...
      <div className="rounded-3xl bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 dark:from-white/[0.08] dark:via-white/[0.04] dark:to-white/[0.08] border border-slate-700 dark:border-white/10 p-6 sm:p-7 shadow-lg">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setReceiptOpen(true)} className="flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-[11px] font-bold text-white/60 hover:bg-white/10 hover:text-white transition-all">
      ...
      {data.followups && data.followups.length > 0 && (
        <div className="rounded-3xl border border-slate-200 dark:border-white/10 bg-white dark:bg-white/[0.02] p-5 sm:p-6 shadow-sm">
...
function ActionItem({ text, detail, tryThis }) {
  ...
  if (!text) return null;
  return (
    <div className="rounded-2xl border border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/[0.02] overflow-hidden">
      <div className="flex items-start gap-3 p-3.5">
        <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-md border border-cyan-400/40 bg-cyan-50 dark:bg-cyan-500/10">
          <Check className="h-3 w-3 text-cyan-600 dark:text-cyan-400" />
        </span>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold text-slate-900 dark:text-white/95 leading-snug">{text}</p>
          {tryThis && <p className="text-xs text-slate-600 dark:text-white/60 mt-1"><strong className="text-cyan-700 dark:text-cyan-300">Try:</strong> {tryThis}</p>}
        </div>
        {detail && (
          <button onClick={() => setOpen(!open)} className="rounded-full border border-slate-200 dark:border-white/10 bg-white dark:bg-white/10 px-2.5 py-0.5 text-[10px] font-bold text-cyan-700 dark:text-cyan-300 hover:bg-cyan-50 dark:hover:bg-cyan-400/20 transition-colors font-mono shrink-0">
            Why?
          </button>
        )}
      </div>
        ...
        {open && detail && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
            <div className="px-3.5 pb-3.5 pl-12">
              <div className="rounded-xl border border-cyan-300 dark:border-cyan-400/20 bg-cyan-50 dark:bg-black/60 p-3 text-[11px] text-cyan-900 dark:text-cyan-200">
