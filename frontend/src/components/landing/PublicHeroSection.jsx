...
export default function PublicHeroSection() {
  ...
  const { openCheckoutModal, openLoginModal } = useAuth();
  const [query, setQuery] = useState("");
  const [activeTab, setActiveTab] = useState("mechanism"); // "mechanism" | "evidence" | "dosage" | "safety"
  ...
  return (
    <div className="relative pt-6 sm:pt-10 pb-16 space-y-12">
      ...
      <div className="pointer-events-none absolute left-1/2 top-10 -z-10 h-[500px] w-[500px] -translate-x-1/2 rounded-full bg-cyan-500/10 blur-[140px]" />
      <div className="pointer-events-none absolute right-1/4 top-1/3 -z-10 h-[400px] w-[400px] rounded-full bg-emerald-500/10 blur-[150px]" />
      ...
      <div className="mx-auto max-w-4xl text-center space-y-5 px-4">
        ...
        <motion.div
          ...
          className="inline-flex items-center gap-2 rounded-full border border-cyan-400/30 bg-gradient-to-r from-cyan-500/10 via-cyan-400/5 to-emerald-500/10 px-4 py-1.5 shadow-[0_0_20px_rgba(6,182,212,0.15)]"
        >
          <Sparkles className="h-3.5 w-3.5 text-cyan-400" />
          <span className="text-xs font-semibold uppercase tracking-[0.2em] text-cyan-300 font-mono">
      ...
      <div className="mx-auto max-w-3xl px-4 space-y-5">
        <div className="rounded-3xl border border-[#1E2E42] bg-[#0E141D]/90 p-4 sm:p-6 shadow-2xl backdrop-blur-xl space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-xs font-mono text-[#94A3B8]">
              <span className="flex h-2 w-2 rounded-full bg-emerald-400 animate-pulse" />
              <span className="text-white font-semibold">Interactive Sandbox:</span>
              <span>Try 1 Free Scientific Deep-Dive</span>
            </div>
            <span className="hidden sm:inline-block text-[10px] font-mono uppercase tracking-wider text-[#64748B] border border-white/10 px-2 py-0.5 rounded-full">
              No Card Required
            </span>
          </div>
          ...
          >
            ...
            <input
              ...
              className="w-full rounded-2xl border border-[#1E293B] bg-black/60 pl-12 pr-28 py-3.5 text-sm text-white placeholder-[#64748B] focus:border-cyan-400 focus:outline-none focus:ring-1 focus:ring-cyan-400 transition-all font-mono"
            />
            <button
              data-testid="hero-free-trial-submit"
              type="submit"
              disabled={loading}
              className="absolute right-2 flex items-center gap-1.5 rounded-xl bg-cyan-400 px-4 py-2 text-xs font-bold text-black hover:bg-cyan-300 transition-all shadow-[0_0_15px_rgba(6,182,212,0.3)] disabled:opacity-50"
            >
              {loading ? (
                <span>Analyzing...</span>
              ) : (
          ...
          <div className="flex flex-wrap items-center gap-2 pt-1">
            <span className="text-[11px] font-mono text-[#64748B]">Starter chips:</span>
            {STARTER_CHIPS.map((chip, idx) => (
              <button
                key={idx}
                data-testid={`hero-starter-chip-${idx}`}
                onClick={() => handleChipClick(chip)}
                className="flex items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.03] px-3 py-1 text-xs text-[#CBD5E1] hover:border-cyan-400/50 hover:bg-cyan-400/10 hover:text-white transition-all group"
              >
                <span className="text-[10px] text-cyan-400/70 font-mono">[{chip.tag}]</span>
        ...
        {activeResult && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            className="rounded-3xl border border-[#1E2E42] bg-[#0E141D] p-5 sm:p-7 shadow-2xl space-y-6"
          >
            {/* Header Result Line */}
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 border-b border-[#1E293B] pb-5">
              ...
              {/* Science Score Badge */}
              <div className="shrink-0 flex items-center sm:flex-col items-end gap-2 sm:gap-1 rounded-2xl border border-cyan-400/30 bg-cyan-400/10 px-4 py-2.5">
                ...
                <span className="text-[9px] font-bold uppercase tracking-widest text-cyan-300 font-mono">
                  Science Score
                </span>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex flex-wrap gap-1.5 border-b border-[#1E293B] pb-2">
                {[
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-1.5 rounded-xl px-3.5 py-2 text-xs font-semibold transition-all ${
                        isActive
                          ? "bg-cyan-400/15 text-cyan-300 border border-cyan-400/40 shadow-sm"
              ...
              {/* Tab Details Content */}
              <div className="rounded-2xl border border-white/5 bg-black/40 p-4 sm:p-5 text-xs sm:text-sm text-white/90 leading-relaxed font-light min-h-[90px]">
                {activeTab === "mechanism" && (
            ...
            <div className="rounded-2xl border border-amber-500/40 bg-gradient-to-r from-amber-500/15 via-[#0E141D] to-amber-500/10 p-4 sm:p-5 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex items-start gap-3 text-left">
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-amber-400/20 border border-amber-400/40 text-amber-300">
                ...
                <div>
                  ...
                  <div className="text-[11px] text-[#94A3B8] font-light mt-0.5">
              ...
              <button
                ...
                className="shrink-0 w-full sm:w-auto flex items-center justify-center gap-2 rounded-xl bg-amber-400 px-5 py-2.5 text-xs font-bold text-black hover:bg-amber-300 hover:scale-105 transition-all shadow-[0_0_20px_rgba(245,158,11,0.3)] font-mono uppercase tracking-wider"
              >
